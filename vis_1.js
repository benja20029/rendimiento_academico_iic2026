...
// Define color scale
const colorScale = defineColorScale(["red", "yellow", "green"], [4.9, 5.5,  6.2]);
...
// Create dropdown
const dropdown = createDropdown("#vis_mapa", "select", "svg", "typeSelect", "absolute", "270px", "700px", ["Predeterminado", "Municipal", "Particular Subvencionado", "Particular Pagado"], "200px", "30px");


// Load grade data
d3.csv("resumen_rendimiento_2010-2022.csv").then(function(gradeData) {

    var averageGrades = calculateAverages(gradeData, "Predeterminado");

    // Load and draw map
    d3.json("chile.json").then(function(topo) {
        topology = topo;
        drawMap(averageGrades, topo, SVG1, path, colorScale, tooltip, "Predeterminado");
        dropdown.on("change", function() {
            SVG1.selectAll(".region").remove();
            const selectedType = d3.select(this).node().value;
            var averages = calculateAverages(gradeData, selectedType);
...
function calculateAverages(gradeData, selectedType) {
    const averages = gradeData.reduce((acc, curr) => {
        if (selectedType === "Predeterminado" || curr.COD_DEPE === selectedType) {
    ...
    regions.append("text")
        ...
        .attr("transform", function(d) {
            ...
            if (isNaN(centroid[0]) || isNaN(centroid[1])) {
                // If invalid, fall back to the center of the SVG
                centroid = [svg.attr("width") / 2, svg.attr("height") / 2];
            }
            // Specific adjustments for certain regions
